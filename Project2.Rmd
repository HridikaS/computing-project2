...
```{r}
gg <- df %>%
    ggplot(aes(year, temperature, group = temp_type, color = temp_type)) + 
    geom_line() + 
    facet_wrap(~city_name)

gg <- gg + labs(
        title = "Minimum and Maximum Temperature Overtime in Five Australian Cities",
...
gg_function <- function(df, city_name, year) {
  if (!(city_name %in% df$city_name) | # Error message for invalid inputs
      !(year %in% df$year)) {
    stop("Error: Invalid input. Please check the data and input parameters before trying again.")
  }
  
  df %>%
    filter(city_name == !!city_name, year == !!year) %>%  # Filter data based on the city and year
    ggplot(aes(log(rainfall))) +  # Plot histogram for the log of rainfall
    geom_histogram() +  
    labs(title = paste("Rainfall Distribution in", city_name, "for the year", year),
         x = "Log of Rainfall",
         y = "Frequency")
}

gg_function(df, "PERTH", 2000) # Call
```
